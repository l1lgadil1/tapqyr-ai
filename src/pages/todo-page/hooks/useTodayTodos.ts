...
/**
 * Custom hook to fetch today's todos
 */
export function useTodayTodos(): UseTodayTodosReturn {
  ...
  const fetchTodos = useCallback(async (force = false) => {
    ...
    try {
      const result = await todoApi.getTodayTodos();
      setTodos(result.todos);
      setTotalItems(result.totalItems);
      initialLoadRef.current = true;
    } catch (err) {
      setError('Failed to fetch today\'s todos');
